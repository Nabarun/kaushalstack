...
// Positions around a 260×400 SVG column (left panel)
const POSITIONS = [
  { x: 130, y: 50 },
  { x: 230, y: 130 },
  { x: 190, y: 240 },
...
export default function RoundTablePage() {
  ...
  const [focusedIdx, setFocusedIdx] = useState(0); // which response is shown in right panel
  ...
  // Scroll right panel when focused response changes
  useEffect(() => {
    if (feedRef.current) {
  ...
  // Auto-focus last response when new ones arrive
  useEffect(() => {
    if (responses.length > 0) {
      setFocusedIdx(responses.length - 1);
    }
  }, [responses.length]);

  async function run(q) {
    const query = (q || prompt).trim();
    if (!query || loading || agents.length === 0) return;
  ...
  const focusedColor    = focusedAgent ? PALETTE[focusedResponse.agentIdx] || PALETTE[0] : null;
      ...
      <div style={{ background: '#080a0e', minHeight: '100vh', color: '#e8eaf0' }} className="flex flex-col">
        ...
        <div style={{ background: '#0d0f16', borderBottom: '1px solid #1e2130' }}
          ...
          <div className="flex items-center gap-3">
            ...
            {loading && (
              <motion.span
                style={{ background: '#12141c', border: '1px solid #1e2130', color: '#5b8dee' }}
                className="text-xs font-mono px-3 py-1 rounded-full"
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="relative" style={{ width: 260, height: 300 }}>
                  ...
                  <motion.div
                    animate={
                      activeIdx === i ? {
                        ...
                        scale: 1.15,
                        borderColor: `${a.color}99`,
                      } : focusedResponse?.agentIdx === i ? {
                        boxShadow: `0 0 0 2px ${a.color}60`,
                        scale: 1.08,
              ...
              <motion.div
                style={{
                  ...
                  width: 36, height: 36,
                  background: '#0f1118',
                  border: `1px solid ${activeIdx >= 0 ? `${PALETTE[activeIdx]}44` : '#1e2130'}`,
                  borderRadius: '50%',
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ textAlign: 'left', flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 10, fontWeight: 600, color: isFocused ? a.color : '#4a4f60', letterSpacing: '0.03em' }}>
                        {a.agent_name}
                      </div>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {responses.length === 0 && !loading ? (
                  ...
                  >
                    {activeIdx >= 0 && agents[activeIdx] && (
                      <div className="flex flex-col items-center gap-3">
                        <motion.img
                          src={avatarUrl(agents[activeIdx].agent_name)}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mb-5">
                      <img
                        src={avatarUrl(focusedAgent?.agent_name || focusedResponse.name)}
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: '#4a4f60', fontFamily: 'monospace' }}>
                          {focusedAgent?.category || ''}
                          {focusedAgent?.name ? ` · ${focusedAgent.name}` : ''}
                        </div>
                      </div>
                    </div>
            ...
            <div style={{ borderTop: '1px solid #1e2130', background: '#0a0c12' }} className="px-5 py-4 space-y-3">
              ...
              {/* Input */}
              <div style={{ background: '#12141c', border: '1px solid #1e2130', borderRadius: 12 }}
                ...
                <input
                  ...
                  style={{
                    ...
                    color: '#e8eaf0', fontSize: 13, fontFamily: 'Syne, sans-serif',
                  }}
                />
                <button
                  onClick={() => run()}
                  ...
                  style={{
                    background: loading || !prompt.trim() ? '#1e2130' : '#5b8dee',

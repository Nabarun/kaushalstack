...
export const SEED_TEAMS = [
    ...
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/j4e and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: register→admin approval→connection→chat→project broadcast→bid→award. Negative: bad login, duplicate bid, unauthorized admin route access, unapproved-member actions, SHOW_PROJECTS flag off. Self-contained (SQLite) — reference implementation for the other teams.' },
    ...
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/mrnmr and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: OAuth connect, campaign generation, scheduled post fires to FB/IG/LinkedIn, token metering. Negative: expired LinkedIn API version, revoked/expired OAuth token, budget cap hit, scheduler misfire. Mock Facebook Graph, Instagram, LinkedIn and kaushalstack partner APIs — no live calls.' },
    ...
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/lakshyan and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: full lifecycle RFQ→bids→award→PO→invoice→payment→QC→close against a mocked OpenProcure API. Negative: invalid order-state transition, expired JWT, spec-drift fields (company_name vs organization_name), duplicate seat signup. Never hit production OpenProcure from the daily run.' },
    ...
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/consciousconnections and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: assessment complete→persist→resume, round-table→spec→creative pipeline, PDF export, email send. Negative: missing KS credentials ("portal not configured"), budget cap breached, SMTP failure, assessment state loss on refresh. Mock kaushalstack partner API and SMTP.' },
    ...
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/vajrahasta and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: portal login, campaign pages and assets serve, environment reports running. Negative: missing partner record, environment down, auth failure. Mock PocketBase and KS platform APIs; include a live uptime ping against the real portal as a separate non-blocking check.' },
    ...
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/enrollengineer and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: ingest fixture audio→transcribe→translate→index→ask→cited answer with timestamps; FAQ tabs. Negative: Sarvam STT failure, audio >2h limit, empty index query, missing/invalid API keys. Mock Sarvam and OpenAI entirely; keep a tiny golden-answer set for citation accuracy.' },
    ...
    },
    {
        ...
        project: '~/Projects/royalinteriors — Seema Jalan, Pune (interiorroyal.in cutover pending)',
        mission: 'Full-screen hero site with Riya, the AI chat concierge — strict knowledge-base answers, escalation to an assessment inbox, instrumented funnels, admin portal with embedded Card Studio.',
        team: [
            { id: 'ri-tanvi', agent_name: 'Tanvi', name: 'Tech Lead', category: 'engineering', associated_tech_skills: 'Next.js 16 App Router, Prisma, Tailwind 4, sprint planning', why: 'The site shipped in one sprint — she keeps the engineering pace while the six marketing agents (Aarav, Riya, Meera, Kabir, Ishita, Dev) drive the business side.', description: 'Runs sprint planning for the Royal Interiors platform, owns the DNS cutover and deploy pattern, and briefs the CEO. Coordinates with the client-facing agent team without overlapping it.' },
            { id: 'ri-nikhil', agent_name: 'Nikhil', name: 'Full-stack Engineer', category: 'engineering', associated_tech_skills: 'Next.js, Prisma + SQLite, analytics instrumentation, admin UX', why: 'Owns the site, the admin portal, and the dual-path event funnels the dashboards depend on.', description: 'Owns the public site and admin portal end-to-end: assessment inbox workflow, analytics dashboards (j4e-pattern beacon + server-side writes), profile/auth, and the embedded Card Studio.' },
            { id: 'ri-priya', agent_name: 'Priya', name: 'Conversation & AI Engineer', category: 'engineering', associated_tech_skills: 'Anthropic API, prompt caching, knowledge-base curation, escalation flows', why: 'Riya answering strictly from the knowledge base — and escalating instead of guessing prices — IS the product promise.', description: 'Owns the /api/chat pipeline: knowledge.ts freshness, prompt-cache hit rate, the escalation marker path into Inquiry(status=escalated), and the no-API-key graceful mode where every question still lands in the inbox.' },
            { id: 'ri-sahil', agent_name: 'Sahil', name: 'QA & Mocks Engineer', category: 'engineering', associated_tech_skills: 'node --test, Anthropic/SMTP mocking, Prisma test fixtures, negative-path testing', why: 'A client-facing AI concierge that starts guessing prices is worse than no concierge — the daily suite is the guard rail.', description: 'Builds the daily test dashboard in KaushalStackTestFramework/suites/royalinteriors with Anthropic and SMTP mocked: chat answers, escalation capture, event ingestion, admin auth, and every env-gated fallback.' },
        ],
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'planned', detail: 'Suite to create at KaushalStackTestFramework/suites/royalinteriors (only team without one). Happy: chat answers from lib/knowledge.ts, escalated question creates Inquiry(status=escalated), event beacon ingestion, admin login, assessment reply flow. Negative: missing ANTHROPIC_API_KEY (chat must still capture to inbox — documented behavior), SMTP absent (mailto fallback), bad admin login, malformed beacon payload. Mock Anthropic and SMTP; throwaway SQLite.' },
            { title: 'DNS cutover to interiorroyal.in', type: 'infra', priority: 'P0', status: 'planned', detail: 'Everything server-side is already in place: Traefik routes interiorroyal.in + www + admin.interiorroyal.in to the site, and proxy.ts does the host routing (admin subdomain → /admin). The ONLY missing piece is DNS — interiorroyal.in still points at Hostinger parking (2.57.91.91, confirmed 26 Jul). In the Hostinger DNS zone: A record @ → 187.127.147.87, plus A records for www and admin. Nothing to build; this is a registrar-panel change.' },
            { title: 'Change the seeded admin credentials', type: 'security', priority: 'P1', status: 'planned', detail: 'The seeded login (interiorroyal4@gmail.com / royal@admin2026) is printed in the README. Fine while the site is dark; must be rotated in Admin → Profile before the domain goes live.' },
            { title: 'SMTP setup for escalation emails', type: 'feature', priority: 'P1', status: 'backlog', detail: 'Escalation notifications and assessment replies are env-gated on SMTP_* and currently fall back to mailto. Configure real SMTP before launch so Seema gets emailed when Riya escalates.' },
            { title: 'Knowledge-base update workflow', type: 'feature', priority: 'P2', status: 'backlog', detail: 'Riya answers strictly from lib/knowledge.ts, so prices/services drift means a code deploy. Give the admin a way to review and update knowledge without an engineer.' },
            { title: 'SQLite backup for ri_data volume', type: 'infra', priority: 'P2', status: 'backlog', detail: 'Inquiries and analytics live in one SQLite file in the ri_data volume — schedule a dump before the site starts taking real leads.' },
        ],
        report: 'The site shipped whole: hero photography, Riya chat with strict knowledge-base answering and escalation, instrumented funnels, admin with assessment inbox and embedded Studio — plus six private marketing agents registered in kaushalstack. We are the only team without a test suite, so that P0 comes first. Second P0 is the DNS cutover: interiorroyal.in is still parked at Hostinger, and the seeded README credentials must rotate before it goes live. Brainstorm ask: should Riya’s escalated-question transcripts feed Kabir (lead qualification) automatically?',
    },
    {
        ...
        items: [
            { title: 'Test dashboard: positive & negative flows, daily', type: 'test', priority: 'P0', status: 'in_progress', detail: 'Happy path SHIPPED in KaushalStackTestFramework/suites/kaushalstack and reporting daily to the admin Test dashboard. Negative flows still to write. Full scope — happy: roundtable→spec pipeline, partner CRUD, marketplace subscription gating by paid_until, sprint-board CRUD + seed idempotency. Negative: credit-cap 402, suspended partner, unpaid subscription hidden, unauthenticated admin routes. Mock all LLM providers; throwaway PocketBase instance.' },
            { title: 'Daily test-run scheduler + ingest', type: 'infra', priority: 'P1', status: 'done', detail: 'DONE. KaushalStackTestFramework/run_daily.sh runs every suite and POSTs one results.json to /admin/sprints/test-report, which fans it out into a test-run row per team. Scheduled with launchd (install_schedule.sh) on the developer machine rather than the VPS, because the suites exercise the local project repos — and launchd over cron so a run missed while the laptop sleeps still fires on wake.' },
            { title: 'Test dashboard in admin (/admin/tests)', type: 'feature', priority: 'P1', status: 'done', detail: 'DONE. Portfolio health tiles, one card per project with a 14-run trend strip, and per-test rows showing the assertion error inline. Red projects sort to the top and auto-expand; a latest run older than 36h is flagged stale so a silently-dead scheduler is visible rather than looking green.' },
            { title: 'Negative-flow coverage across every suite', type: 'test', priority: 'P1', status: 'planned', detail: 'Every team now has one happy path green. Next is the negative flows named in each team’s mandated P0. Writing the first happy tests already surfaced three confirmed bugs, so treat this as bug-finding work rather than paperwork.' },
